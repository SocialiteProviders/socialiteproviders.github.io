...
* The listener that you add for this provider is `'SocialiteProviders\\DeviantArt\\DeviantartExtendSocialite@handle',`.
...
return Socialite::with('deviantART')->stateless(false)->redirect();

// to use stateless
return Socialite::with('deviantART')->stateless()->redirect();
```
...
return Socialite::with('deviantART')->setConfig($config)->redirect();
```
