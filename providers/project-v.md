...
* The listener that you add for this provider is `'SocialiteProviders\\ProjectV\\ProjectVExtendSocialite@handle',`.
...
```php
'projectv' => [
    'client_id' => env('PROJECTV_KEY'),
    'client_secret' => env('PROJECTV_SECRET'),
...
return Socialite::with('ProjectV')->stateless(false)->redirect();

// to use stateless
return Socialite::with('ProjectV')->stateless()->redirect();
```
...
return Socialite::with('ProjectV')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('ProjectV')->user();
