...
* The listener that you add for this provider is `'SocialiteProviders\\Instagram\\InstagramExtendSocialite@handle',`.
...
// to use stateless
return Socialite::with('Instagram')->stateless()->redirect();
```
...
return Socialite::with('Instagram')->setConfig($config)->redirect();
```
