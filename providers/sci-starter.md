...
* The listener that you add for this provider is `'SocialiteProviders\\SciStarter\\SciStarterExtendSocialite@handle',`.
...
```php
'scistarter' => [
    'client_id' => env('SCISTARTER_KEY'),
    'client_secret' => env('SCISTARTER_SECRET'),
...
return Socialite::with('SciStarter')->stateless(false)->redirect();

// to use stateless
return Socialite::with('SciStarter')->stateless()->redirect();
```
...
return Socialite::with('SciStarter')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('SciStarter')->user();
