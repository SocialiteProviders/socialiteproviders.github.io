...
* The listener that you add for this provider is `'SocialiteProviders\\Xing\\XingExtendSocialite@handle',`.
...
return Socialite::with('Xing')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Xing')->stateless()->redirect();
```
...
return Socialite::with('Xing')->setConfig($config)->redirect();
```
