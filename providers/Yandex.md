...
* The listener that you add for this provider is `'SocialiteProviders\\Yandex\\YandexExtendSocialite@handle',`.
...
// to use stateless
return Socialite::with('Yandex')->stateless()->redirect();
```
...
return Socialite::with('Yandex')->setConfig($config)->redirect();
```
