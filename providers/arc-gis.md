...
* The listener that you add for this provider is `'SocialiteProviders\\ArcGIS\\ArcGISExtendSocialite@handle',`.
...
return Socialite::with('ArcGIS')->stateless(false)->redirect();

// to use stateless
return Socialite::with('ArcGIS')->stateless()->redirect();
```
...
return Socialite::with('ArcGIS')->setConfig($config)->redirect();
```
