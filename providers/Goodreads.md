...
* The listener that you add for this provider is `'SocialiteProviders\\Goodreads\\GoodreadsExtendSocialite@handle',`.
...
return Socialite::with('Goodreads')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Goodreads')->stateless()->redirect();
```
...
return Socialite::with('Goodreads')->setConfig($config)->redirect();
```
