...
* The listener that you add for this provider is `'SocialiteProviders\\AppNet\\AppNetExtendSocialite@handle',`.
...
return Socialite::with('App.net')->stateless(false)->redirect();

// to use stateless
return Socialite::with('App.net')->stateless()->redirect();
```
...
return Socialite::with('App.net')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('App.net')->user();
