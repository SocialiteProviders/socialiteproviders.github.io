...
* The listener that you add for this provider is `'SocialiteProviders\\Jawbone\\JawboneExtendSocialite@handle',`.
...
protected $listen = [
    \SocialiteProviders\Manager\SocialiteWasCalled::class => [
        ...
        'SocialiteProviders\\Jawbone\\JawboneExtendSocialite@handle',
    ],
];
```
...
return Socialite::with('Jawbone')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Jawbone')->stateless()->redirect();
```
...
return Socialite::with('Jawbone')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Jawbone')->user();
