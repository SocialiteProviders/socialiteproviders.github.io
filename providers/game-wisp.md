...
composer require socialiteproviders/gamewisp
```
...
* The listener that you add for this provider is `'SocialiteProviders\\GameWisp\\GameWispExtendSocialite@handle',`.
...
protected $listen = [
    \SocialiteProviders\Manager\SocialiteWasCalled::class => [
        ...
        'SocialiteProviders\\GameWisp\\GameWispExtendSocialite@handle',
    ],
];
```
...
```php
'gamewisp' => [
    'client_id' => env('GAMEWISP_KEY'),
    'client_secret' => env('GAMEWISP_SECRET'),
    'redirect' => env('GAMEWISP_REDIRECT_URI')
],
```
...
```php
return Socialite::with('GameWisp')->redirect();
```
...
return Socialite::with('GameWisp')->stateless(false)->redirect();

// to use stateless
return Socialite::with('GameWisp')->stateless()->redirect();
```
...
return Socialite::with('GameWisp')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('GameWisp')->user();
