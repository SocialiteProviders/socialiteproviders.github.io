...
* The listener that you add for this provider is `'SocialiteProviders\\Disqus\\DisqusExtendSocialite@handle',`.
...
return Socialite::with('Disqus')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Disqus')->stateless()->redirect();
```
...
return Socialite::with('Disqus')->setConfig($config)->redirect();
```
