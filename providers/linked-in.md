...
* The listener that you add for this provider is `'SocialiteProviders\\LinkedIn\\LinkedInExtendSocialite@handle',`.
...
// to use stateless
return Socialite::with('LinkedIn')->stateless()->redirect();
```
...
return Socialite::with('LinkedIn')->setConfig($config)->redirect();
```
