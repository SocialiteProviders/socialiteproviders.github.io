...
* The listener that you add for this provider is `'SocialiteProviders\\Dailymotion\\DailymotionExtendSocialite@handle',`.
...
return Socialite::with('Dailymotion')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Dailymotion')->stateless()->redirect();
```
...
return Socialite::with('Dailymotion')->setConfig($config)->redirect();
```
