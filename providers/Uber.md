...
* The listener that you add for this provider is `'SocialiteProviders\\Uber\\UberExtendSocialite@handle',`.
...
return Socialite::with('Uber')->stateless(false)->redirect();
...
return Socialite::with('Uber')->setConfig($config)->redirect();
```
