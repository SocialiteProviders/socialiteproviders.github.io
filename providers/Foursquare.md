...
* The listener that you add for this provider is `'SocialiteProviders\\Foursquare\\FoursquareExtendSocialite@handle',`.
...
return Socialite::with('Foursquare')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Foursquare')->stateless()->redirect();
```
...
return Socialite::with('Foursquare')->setConfig($config)->redirect();
```
