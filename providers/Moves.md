...
composer require socialiteproviders/moves
```
...
* The listener that you add for this provider is `'SocialiteProviders\\Moves\\MovesExtendSocialite@handle',`.
...
protected $listen = [
    \SocialiteProviders\Manager\SocialiteWasCalled::class => [
        ...
        'SocialiteProviders\\Moves\\MovesExtendSocialite@handle',
    ],
];
```
...
```php
'moves' => [
    'client_id' => env('MOVES_KEY'),
    'client_secret' => env('MOVES_SECRET'),
    'redirect' => env('MOVES_REDIRECT_URI')
],
```
...
```php
return Socialite::with('Moves')->redirect();
```
...
return Socialite::with('Moves')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Moves')->stateless()->redirect();
```
...
return Socialite::with('Moves')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Moves')->user();
