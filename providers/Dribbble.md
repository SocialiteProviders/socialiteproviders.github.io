...
* The listener that you add for this provider is `'SocialiteProviders\\Dribbble\\DribbbleExtendSocialite@handle',`.
...
return Socialite::with('Dribbble')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Dribbble')->stateless()->redirect();
```
...
return Socialite::with('Dribbble')->setConfig($config)->redirect();
```
