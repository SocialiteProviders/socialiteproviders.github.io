...
* The listener that you add for this provider is `'SocialiteProviders\\QuickBooks\\QuickBooksExtendSocialite@handle',`.
...
return Socialite::with('QuickBooks')->stateless(false)->redirect();

// to use stateless
return Socialite::with('QuickBooks')->stateless()->redirect();
```
...
return Socialite::with('QuickBooks')->setConfig($config)->redirect();
```
