...
* The listener that you add for this provider is `'SocialiteProviders\\Strava\\StravaExtendSocialite@handle',`.
...
return Socialite::with('Strava')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Strava')->stateless()->redirect();
```
...
return Socialite::with('Strava')->setConfig($config)->redirect();
```
