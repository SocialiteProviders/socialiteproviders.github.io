...
* The listener that you add for this provider is `'SocialiteProviders\\StackExchange\\StackExchangeExtendSocialite@handle',`.
...
return Socialite::with('StackExchange')->stateless(false)->redirect();

// to use stateless
return Socialite::with('StackExchange')->stateless()->redirect();
```
