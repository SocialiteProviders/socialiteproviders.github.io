...
* The listener that you add for this provider is `'SocialiteProviders\\Pushbullet\\PushbulletExtendSocialite@handle',`.
...
return Socialite::with('Pushbullet')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Pushbullet')->stateless()->redirect();
```
...
return Socialite::with('Pushbullet')->setConfig($config)->redirect();
```
