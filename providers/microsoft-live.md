...
* The listener that you add for this provider is `'SocialiteProviders\\Live\\LiveExtendSocialite@handle',`.
...
return Socialite::with('live')->stateless(false)->redirect();

// to use stateless
return Socialite::with('live')->stateless()->redirect();
```
...
return Socialite::with('live')->setConfig($config)->redirect();
```
