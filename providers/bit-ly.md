...
* The listener that you add for this provider is `'SocialiteProviders\\Bitly\\BitlyExtendSocialite@handle',`.
...
```php
'bit.ly' => [
    'client_id' => env('BIT.LY_KEY'),
    'client_secret' => env('BIT.LY_SECRET'),
    'redirect' => env('BIT.LY_REDIRECT_URI')
],
```
...
return Socialite::with('Bit.ly')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Bit.ly')->stateless()->redirect();
```
...
return Socialite::with('Bit.ly')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Bit.ly')->user();
