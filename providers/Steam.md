...
* The listener that you add for this provider is `'SocialiteProviders\\Steam\\SteamExtendSocialite@handle',`.
...
return Socialite::with('Steam')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Steam')->stateless()->redirect();
```
