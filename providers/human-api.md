...
```php
'human api' => [
    'client_id' => env('HUMAN API_KEY'),
    'client_secret' => env('HUMAN API_SECRET'),
    'redirect' => env('HUMAN API_REDIRECT_URI')
],
```
...
```php
return Socialite::with('Human API')->redirect();
```
...
return Socialite::with('Human API')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Human API')->stateless()->redirect();
```
...
return Socialite::with('Human API')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Human API')->user();
