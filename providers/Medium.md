...
* The listener that you add for this provider is `'SocialiteProviders\\Medium\\MediumExtendSocialite@handle',`.
...
return Socialite::with('Medium')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Medium')->stateless()->redirect();
```
...
return Socialite::with('Medium')->setConfig($config)->redirect();
```
