...
* The listener that you add for this provider is `'SocialiteProviders\\Yahoo\\YahooExtendSocialite@handle',`.
...
return Socialite::with('Yahoo')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Yahoo')->stateless()->redirect();
```
...
return Socialite::with('Yahoo')->setConfig($config)->redirect();
```
