...
* The listener that you add for this provider is `'SocialiteProviders\\Podio\\PodioExtendSocialite@handle',`.
...
return Socialite::with('Podio')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Podio')->stateless()->redirect();
```
...
return Socialite::with('Podio')->setConfig($config)->redirect();
```
