...
* The listener that you add for this provider is `'SocialiteProviders\\Untappd\\UntappdExtendSocialite@handle',`.
...
return Socialite::with('Untappd')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Untappd')->stateless()->redirect();
```
...
return Socialite::with('Untappd')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Untappd')->user();
