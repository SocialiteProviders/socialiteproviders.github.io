...
* The listener that you add for this provider is `'SocialiteProviders\\TVShowTime\\TVShowTimeExtendSocialite@handle',`.
...
```php
'tvshowtime' => [
    'client_id' => env('TVSHOWTIME_KEY'),
    'client_secret' => env('TVSHOWTIME_SECRET'),
...
return Socialite::with('TVShowTime')->stateless(false)->redirect();

// to use stateless
return Socialite::with('TVShowTime')->stateless()->redirect();
```
...
return Socialite::with('TVShowTime')->setConfig($config)->redirect();
```
