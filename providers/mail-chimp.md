...
* The listener that you add for this provider is `'SocialiteProviders\\MailChimp\\MailChimpExtendSocialite@handle',`.
...
return Socialite::with('MailChimp')->stateless(false)->redirect();

// to use stateless
return Socialite::with('MailChimp')->stateless()->redirect();
```
...
return Socialite::with('MailChimp')->setConfig($config)->redirect();
```
