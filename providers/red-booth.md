...
* The listener that you add for this provider is `'SocialiteProviders\\RedBooth\\RedBoothExtendSocialite@handle',`.
...
return Socialite::with('RedBooth')->stateless(false)->redirect();

// to use stateless
return Socialite::with('RedBooth')->stateless()->redirect();
```
...
return Socialite::with('RedBooth')->setConfig($config)->redirect();
```
