...
* The listener that you add for this provider is `'SocialiteProviders\\RunKeeper\\RunKeeperExtendSocialite@handle',`.
...
return Socialite::with('RunKeeper')->stateless(false)->redirect();

// to use stateless
return Socialite::with('RunKeeper')->stateless()->redirect();
```
...
return Socialite::with('RunKeeper')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('RunKeeper')->user();
