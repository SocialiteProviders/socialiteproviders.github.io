...
* The listener that you add for this provider is `'SocialiteProviders\\Envato\\EnvatoExtendSocialite@handle',`.
...
return Socialite::with('Envato')->stateless(false)->redirect();
...
return Socialite::with('Envato')->setConfig($config)->redirect();
```
