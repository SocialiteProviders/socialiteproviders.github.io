...
* The listener that you add for this provider is `'SocialiteProviders\\Paymill\\PaymillExtendSocialite@handle',`.
...
return Socialite::with('Paymill')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Paymill')->stateless()->redirect();
```
...
return Socialite::with('Paymill')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Paymill')->user();
