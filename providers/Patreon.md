...
* The listener that you add for this provider is `'SocialiteProviders\\Patreon\\PatreonExtendSocialite@handle',`.
...
return Socialite::with('Patreon')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Patreon')->stateless()->redirect();
```
...
return Socialite::with('Patreon')->setConfig($config)->redirect();
```
