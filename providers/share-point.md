...
* The listener that you add for this provider is `'SocialiteProviders\\SharePoint\\SharePointExtendSocialite@handle',`.
...
return Socialite::with('SharePoint')->stateless(false)->redirect();

// to use stateless
return Socialite::with('SharePoint')->stateless()->redirect();
```
...
return Socialite::with('SharePoint')->setConfig($config)->redirect();
```
