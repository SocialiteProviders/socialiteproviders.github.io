...
* The listener that you add for this provider is `'SocialiteProviders\\Yammer\\YammerExtendSocialite@handle',`.
...
return Socialite::with('Yammer')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Yammer')->stateless()->redirect();
```
...
return Socialite::with('Yammer')->setConfig($config)->redirect();
```
