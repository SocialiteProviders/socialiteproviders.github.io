...
* The listener that you add for this provider is `'SocialiteProviders\\Harvest\\HarvestExtendSocialite@handle',`.
...
return Socialite::with('Harvest')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Harvest')->stateless()->redirect();
```
...
return Socialite::with('Harvest')->setConfig($config)->redirect();
```
