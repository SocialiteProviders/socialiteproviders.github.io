...
* The listener that you add for this provider is `'SocialiteProviders\\Heroku\\HerokuExtendSocialite@handle',`.
...
return Socialite::with('Heroku')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Heroku')->stateless()->redirect();
```
...
return Socialite::with('Heroku')->setConfig($config)->redirect();
```
