...
* The listener that you add for this provider is `'SocialiteProviders\\SoundCloud\\SoundCloudExtendSocialite@handle',`.
...
return Socialite::with('SoundCloud')->stateless(false)->redirect();

// to use stateless
return Socialite::with('SoundCloud')->stateless()->redirect();
```
...
return Socialite::with('SoundCloud')->setConfig($config)->redirect();
```
