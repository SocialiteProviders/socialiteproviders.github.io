...
* The listener that you add for this provider is `'SocialiteProviders\\Dataporten\\DataportenExtendSocialite@handle',`.
...
return Socialite::with('Dataporten')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Dataporten')->stateless()->redirect();
```
...
return Socialite::with('Dataporten')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Dataporten')->user();
