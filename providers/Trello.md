...
* The listener that you add for this provider is `'SocialiteProviders\\Trello\\TrelloExtendSocialite@handle',`.
...
return Socialite::with('Trello')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Trello')->stateless()->redirect();
```
...
return Socialite::with('Trello')->setConfig($config)->redirect();
```
