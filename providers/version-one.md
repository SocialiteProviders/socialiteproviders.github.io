...
* The listener that you add for this provider is `'SocialiteProviders\\VersionOne\\VersionOneExtendSocialite@handle',`.
...
```php
'versionone' => [
    'client_id' => env('VERSIONONE_KEY'),
    'client_secret' => env('VERSIONONE_SECRET'),
...
return Socialite::with('VersionOne')->stateless(false)->redirect();

// to use stateless
return Socialite::with('VersionOne')->stateless()->redirect();
```
...
return Socialite::with('VersionOne')->setConfig($config)->redirect();
```
