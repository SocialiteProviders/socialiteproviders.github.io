...
* The listener that you add for this provider is `'SocialiteProviders\\Trakt\\TraktExtendSocialite@handle',`.
...
return Socialite::with('Trakt')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Trakt')->stateless()->redirect();
```
...
return Socialite::with('Trakt')->setConfig($config)->redirect();
```
