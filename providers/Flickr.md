...
* The listener that you add for this provider is `'SocialiteProviders\\Flickr\\FlickrExtendSocialite@handle',`.
...
return Socialite::with('Flickr')->stateless(false)->redirect();
...
return Socialite::with('Flickr')->setConfig($config)->redirect();
```
