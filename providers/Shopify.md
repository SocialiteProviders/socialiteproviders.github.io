...
* The listener that you add for this provider is `'SocialiteProviders\\Shopify\\ShopifyExtendSocialite@handle',`.
...
return Socialite::with('Shopify')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Shopify')->stateless()->redirect();
```
...
return Socialite::with('Shopify')->setConfig($config)->redirect();
```
