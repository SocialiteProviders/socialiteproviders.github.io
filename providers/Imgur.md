...
* The listener that you add for this provider is `'SocialiteProviders\\Imgur\\ImgurExtendSocialite@handle',`.
...
return Socialite::with('Imgur')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Imgur')->stateless()->redirect();
```
...
return Socialite::with('Imgur')->setConfig($config)->redirect();
```
