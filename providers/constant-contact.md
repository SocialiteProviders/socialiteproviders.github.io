...
* The listener that you add for this provider is `'SocialiteProviders\\ConstantContact\\ConstantContactExtendSocialite@handle',`.
...
return Socialite::with('ConstantContact')->stateless(false)->redirect();

// to use stateless
return Socialite::with('ConstantContact')->stateless()->redirect();
```
...
return Socialite::with('ConstantContact')->setConfig($config)->redirect();
```
