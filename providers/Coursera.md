...
* The listener that you add for this provider is `'SocialiteProviders\\Coursera\\CourseraExtendSocialite@handle',`.
...
```php
'coursera' => [
    'client_id' => env('COURSERA_KEY'),
    'client_secret' => env('COURSERA_SECRET'),
...
return Socialite::with('Coursera')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Coursera')->stateless()->redirect();
```
...
return Socialite::with('Coursera')->setConfig($config)->redirect();
```
