...
* The listener that you add for this provider is `'SocialiteProviders\\Buffer\\BufferExtendSocialite@handle',`.
...
return Socialite::with('Buffer')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Buffer')->stateless()->redirect();
```
...
return Socialite::with('Buffer')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Buffer')->user();
