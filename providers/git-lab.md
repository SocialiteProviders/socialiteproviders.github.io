...
* The listener that you add for this provider is `'SocialiteProviders\\GitLab\\GitLabExtendSocialite@handle',`.
...
return Socialite::with('GitLab')->stateless(false)->redirect();

// to use stateless
return Socialite::with('GitLab')->stateless()->redirect();
```
