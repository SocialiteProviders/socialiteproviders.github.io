...
* The listener that you add for this provider is `'SocialiteProviders\\Deezer\\DeezerExtendSocialite@handle',`.
...
// to use stateless
return Socialite::with('Deezer')->stateless()->redirect();
```
...
return Socialite::with('Deezer')->setConfig($config)->redirect();
```
