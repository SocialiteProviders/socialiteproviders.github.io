...
* The listener that you add for this provider is `'SocialiteProviders\\PayPal\\PayPalExtendSocialite@handle',`.
...
return Socialite::with('PayPal')->stateless(false)->redirect();

// to use stateless
return Socialite::with('PayPal')->stateless()->redirect();
```
...
return Socialite::with('PayPal')->setConfig($config)->redirect();
```
