...
* The listener that you add for this provider is `'SocialiteProviders\\Naver\\NaverExtendSocialite@handle',`.
...
return Socialite::with('Naver')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Naver')->stateless()->redirect();
```
...
return Socialite::with('Naver')->setConfig($config)->redirect();
```
