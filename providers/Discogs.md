...
* The listener that you add for this provider is `'SocialiteProviders\\Discogs\\DiscogsExtendSocialite@handle',`.
...
return Socialite::with('Discogs')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Discogs')->stateless()->redirect();
```
...
return Socialite::with('Discogs')->setConfig($config)->redirect();
```
