...
* The listener that you add for this provider is `'SocialiteProviders\\Pinterest\\PinterestExtendSocialite@handle',`.
...
// to use stateless
return Socialite::with('Pinterest')->stateless()->redirect();
```
...
return Socialite::with('Pinterest')->setConfig($config)->redirect();
```
