...
* The listener that you add for this provider is `'SocialiteProviders\\Yiban\\YibanExtendSocialite@handle',`.
...
```php
'yiban' => [
    'client_id' => env('YIBAN_KEY'),
    'client_secret' => env('YIBAN_SECRET'),
...
return Socialite::with('Yiban')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Yiban')->stateless()->redirect();
```
...
return Socialite::with('Yiban')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Yiban')->user();
