...
* The listener that you add for this provider is `'SocialiteProviders\\Rdio\\RdioExtendSocialite@handle',`.
...
protected $listen = [
    \SocialiteProviders\Manager\SocialiteWasCalled::class => [
        ...
        'SocialiteProviders\\Rdio\\RdioExtendSocialite@handle',
    ],
];
```
...
return Socialite::with('Rdio')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Rdio')->stateless()->redirect();
```
...
return Socialite::with('Rdio')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Rdio')->user();
