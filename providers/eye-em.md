...
* The listener that you add for this provider is `'SocialiteProviders\\EyeEm\\EyeEmExtendSocialite@handle',`.
...
return Socialite::with('EyeEm')->stateless(false)->redirect();

// to use stateless
return Socialite::with('EyeEm')->stateless()->redirect();
```
...
return Socialite::with('EyeEm')->setConfig($config)->redirect();
```
