...
* The listener that you add for this provider is `'SocialiteProviders\\Tumblr\\TumblrExtendSocialite@handle',`.
...
return Socialite::with('Tumblr')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Tumblr')->stateless()->redirect();
```
...
return Socialite::with('Tumblr')->setConfig($config)->redirect();
```
