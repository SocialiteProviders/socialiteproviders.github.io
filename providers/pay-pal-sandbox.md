...
* The listener that you add for this provider is `'SocialiteProviders\\PayPalSandbox\\PayPalSandboxExtendSocialite@handle',`.
...
return Socialite::with('paypal_sandbox')->stateless(false)->redirect();

// to use stateless
return Socialite::with('paypal_sandbox')->stateless()->redirect();
```
...
return Socialite::with('paypal_sandbox')->setConfig($config)->redirect();
```
