...
* The listener that you add for this provider is `'SocialiteProviders\\TwentyThreeAndMe\\TwentyThreeAndMeExtendSocialite@handle',`.
...
return Socialite::with('23andme')->stateless(false)->redirect();

// to use stateless
return Socialite::with('23andme')->stateless()->redirect();
```
...
return Socialite::with('23andme')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('23andme')->user();
