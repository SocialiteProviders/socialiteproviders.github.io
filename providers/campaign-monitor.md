...
* The listener that you add for this provider is `'SocialiteProviders\\CampaignMonitor\\CampaignMonitorExtendSocialite@handle',`.
...
return Socialite::with('CampaignMonitor')->stateless(false)->redirect();

// to use stateless
return Socialite::with('CampaignMonitor')->stateless()->redirect();
```
...
return Socialite::with('CampaignMonitor')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('CampaignMonitor')->user();
