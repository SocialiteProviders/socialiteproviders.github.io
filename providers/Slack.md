...
* The listener that you add for this provider is `'SocialiteProviders\\Slack\\SlackExtendSocialite@handle',`.
...
return Socialite::with('Slack')->stateless(false)->redirect();
...
return Socialite::with('Slack')->setConfig($config)->redirect();
```
