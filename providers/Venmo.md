...
* The listener that you add for this provider is `'SocialiteProviders\\Venmo\\VenmoExtendSocialite@handle',`.
...
return Socialite::with('Venmo')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Venmo')->stateless()->redirect();
```
...
return Socialite::with('Venmo')->setConfig($config)->redirect();
```
