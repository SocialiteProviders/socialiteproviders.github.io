...
* The listener that you add for this provider is `'SocialiteProviders\\Line\\LineExtendSocialite@handle',`.
...
return Socialite::with('LINE')->stateless(false)->redirect();

// to use stateless
return Socialite::with('LINE')->stateless()->redirect();
```
...
return Socialite::with('LINE')->setConfig($config)->redirect();
```
