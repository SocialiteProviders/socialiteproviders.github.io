...
* The listener that you add for this provider is `'SocialiteProviders\\Box\\BoxExtendSocialite@handle',`.
...
return Socialite::with('Box')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Box')->stateless()->redirect();
```
...
return Socialite::with('Box')->setConfig($config)->redirect();
```
