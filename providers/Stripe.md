...
* The listener that you add for this provider is `'SocialiteProviders\\Stripe\\StripeExtendSocialite@handle',`.
...
return Socialite::with('Stripe')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Stripe')->stateless()->redirect();
```
...
return Socialite::with('Stripe')->setConfig($config)->redirect();
```
