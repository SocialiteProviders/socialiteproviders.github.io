...
* The listener that you add for this provider is `'SocialiteProviders\\Meetup\\MeetupExtendSocialite@handle',`.
...
return Socialite::with('Meetup')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Meetup')->stateless()->redirect();
```
...
return Socialite::with('Meetup')->setConfig($config)->redirect();
```
