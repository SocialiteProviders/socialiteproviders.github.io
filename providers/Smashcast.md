...
* The listener that you add for this provider is `'SocialiteProviders\\Smashcast\\SmashcastExtendSocialite@handle',`.
...
```php
'smashcast' => [
    'client_id' => env('SMASHCAST_KEY'),
    'client_secret' => env('SMASHCAST_SECRET'),
...
return Socialite::with('Smashcast')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Smashcast')->stateless()->redirect();
```
...
return Socialite::with('Smashcast')->setConfig($config)->redirect();
```
