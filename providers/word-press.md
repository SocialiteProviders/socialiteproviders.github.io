...
* The listener that you add for this provider is `'SocialiteProviders\\WordPress\\WordPressExtendSocialite@handle',`.
...
return Socialite::with('WordPress')->stateless(false)->redirect();

// to use stateless
return Socialite::with('WordPress')->stateless()->redirect();
```
...
return Socialite::with('WordPress')->setConfig($config)->redirect();
```
