...
* The listener that you add for this provider is `'SocialiteProviders\\xREL\\xRELExtendSocialite@handle',`.
...
return Socialite::with('xREL')->stateless(false)->redirect();

// to use stateless
return Socialite::with('xREL')->stateless()->redirect();
```
...
return Socialite::with('xREL')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('xREL')->user();
