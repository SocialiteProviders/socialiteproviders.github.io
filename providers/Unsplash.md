...
* The listener that you add for this provider is `'SocialiteProviders\\Unsplash\\UnsplashExtendSocialite@handle',`.
...
return Socialite::with('Unsplash')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Unsplash')->stateless()->redirect();
```
...
return Socialite::with('Unsplash')->setConfig($config)->redirect();
```
