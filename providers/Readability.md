...
* The listener that you add for this provider is `'SocialiteProviders\\Readability\\ReadabilityExtendSocialite@handle',`.
...
return Socialite::with('Readability')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Readability')->stateless()->redirect();
```
...
return Socialite::with('Readability')->setConfig($config)->redirect();
```
