...
* The listener that you add for this provider is `'SocialiteProviders\\FiveHundredPixel\\FiveHundredPixelExtendSocialite@handle',`.
...
return Socialite::with('500px')->stateless(false)->redirect();

// to use stateless
return Socialite::with('500px')->stateless()->redirect();
```
...
return Socialite::with('500px')->setConfig($config)->redirect();
```
