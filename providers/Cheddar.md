...
* The listener that you add for this provider is `'SocialiteProviders\\Cheddar\\CheddarExtendSocialite@handle',`.
...
return Socialite::with('Cheddar')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Cheddar')->stateless()->redirect();
```
...
return Socialite::with('Cheddar')->setConfig($config)->redirect();
```
