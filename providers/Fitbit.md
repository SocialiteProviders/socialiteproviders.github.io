...
* The listener that you add for this provider is `'SocialiteProviders\\Fitbit\\FitbitExtendSocialite@handle',`.
...
return Socialite::with('Fitbit')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Fitbit')->stateless()->redirect();
```
...
return Socialite::with('Fitbit')->setConfig($config)->redirect();
```
