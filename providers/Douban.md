...
* The listener that you add for this provider is `'SocialiteProviders\\Douban\\DoubanExtendSocialite@handle',`.
...
```php
'douban' => [
    'client_id' => env('DOUBAN_KEY'),
    'client_secret' => env('DOUBAN_SECRET'),
...
return Socialite::with('Douban')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Douban')->stateless()->redirect();
```
...
return Socialite::with('Douban')->setConfig($config)->redirect();
```
