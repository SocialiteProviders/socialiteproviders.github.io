...
* The listener that you add for this provider is `'SocialiteProviders\\Etsy\\EtsyExtendSocialite@handle',`.
...
return Socialite::with('Etsy')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Etsy')->stateless()->redirect();
```
...
return Socialite::with('Etsy')->setConfig($config)->redirect();
```
