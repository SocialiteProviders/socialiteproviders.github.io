...
* The listener that you add for this provider is `'SocialiteProviders\\AngelList\\AngelListExtendSocialite@handle',`.
...
return Socialite::with('AngelList')->stateless(false)->redirect();

// to use stateless
return Socialite::with('AngelList')->stateless()->redirect();
```
...
return Socialite::with('AngelList')->setConfig($config)->redirect();
```
