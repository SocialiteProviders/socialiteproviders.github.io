...
* The listener that you add for this provider is `'SocialiteProviders\\Twitch\\TwitchExtendSocialite@handle',`.
...
return Socialite::with('Twitch')->stateless(false)->redirect();
...
return Socialite::with('Twitch')->setConfig($config)->redirect();
```
