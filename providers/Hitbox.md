...
composer require socialiteproviders/hitbox
```
...
* The listener that you add for this provider is `'SocialiteProviders\\Hitbox\\HitboxExtendSocialite@handle',`.
...
protected $listen = [
    \SocialiteProviders\Manager\SocialiteWasCalled::class => [
        ...
        'SocialiteProviders\\Hitbox\\HitboxExtendSocialite@handle',
    ],
];
```
...
```php
'hitbox' => [
    'client_id' => env('HITBOX_KEY'),
    'client_secret' => env('HITBOX_SECRET'),
    'redirect' => env('HITBOX_REDIRECT_URI')
],
```
...
```php
return Socialite::with('Hitbox')->redirect();
```
...
return Socialite::with('Hitbox')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Hitbox')->stateless()->redirect();
```
...
return Socialite::with('Hitbox')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Hitbox')->user();
