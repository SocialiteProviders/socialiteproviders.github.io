...
* The listener that you add for this provider is `'SocialiteProviders\\Coinbase\\CoinbaseExtendSocialite@handle',`.
...
return Socialite::with('Coinbase')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Coinbase')->stateless()->redirect();
```
...
return Socialite::with('Coinbase')->setConfig($config)->redirect();
```
