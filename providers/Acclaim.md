...
* The listener that you add for this provider is `'SocialiteProviders\\Acclaim\\AcclaimExtendSocialite@handle',`.
...
return Socialite::with('Acclaim')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Acclaim')->stateless()->redirect();
```
...
return Socialite::with('Acclaim')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Acclaim')->user();
