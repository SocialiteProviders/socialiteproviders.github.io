...
composer require socialiteproviders/mixer
```
...
* The listener that you add for this provider is `'SocialiteProviders\\Mixer\\MixerExtendSocialite@handle',`.
...
```php
return Socialite::with('Mixer')->redirect();
```
...
return Socialite::with('Mixer')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Mixer')->stateless()->redirect();
```
...
return Socialite::with('Mixer')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Mixer')->user();
