...
* The listener that you add for this provider is `'SocialiteProviders\\Jira\\JiraExtendSocialite@handle',`.
...
return Socialite::with('Jira')->stateless(false)->redirect();

// to use stateless
return Socialite::with('Jira')->stateless()->redirect();
```
...
return Socialite::with('Jira')->setConfig($config)->redirect();
```
...
```php
$user = Socialite::driver('Jira')->user();
